...
//Likely need to change all the following : Lets only change the physics List for now.
#include "A2DetectorConstruction.hh"
//#include "PolPhysicsList.hh"

#include "PolPhysicsList.hh" //ChangetoPOL
...
/*
#include "../npol/include/npolDetCon.hh"
#include "../npol/include/PolPhysicsList.hh"
#include "A2PrimaryGeneratorAction.hh"
#include "A2RunAction.hh"
#include "../npol/include/npolEventAction.hh"
#include "../npol/include/npolSteppingAction.hh"
#include "A2SteppingVerbose.hh"
*/
...
//   nameFileMac = "visTutor/gui.mac");
  ...
  A2DetectorConstruction* detector = new A2DetectorConstruction(detSetup); //this one likely needs to be changed to npol version
  //npolDetectorConstruction* detector = new npolDetectorConstruction(detSetup);
  ...
  //runManager->SetUserInitialization(new PolPhysicsList); //ChangetoPOL 
  ...
  A2EventAction* eventaction = new A2EventAction(runaction); //Change?
  //npolEventAction* eventaction = new npolEventAction(runaction);
  eventaction->SetIsInteractive(isInteractive);
  runManager->SetUserAction(eventaction);
  runManager->SetUserAction(new A2SteppingAction(detector, eventaction)); //Change?
  //runManager->SetUserAction(new npolSteppingAction(detector, eventaction));
