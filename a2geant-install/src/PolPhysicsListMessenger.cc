...
{   
  ...
  fposCutCmd->AvailableForStates(G4State_PreInit,G4State_Idle);
   ...
   //CAM 4/3/16 attempting to add in setRegion, CutProt adn RegionCut for POL list
  pCutCmd = new G4UIcmdWithADoubleAndUnit("/Pol/physics/CutProt",this);
  ...
  flistCmd->AvailableForStates(G4State_PreInit,G4State_Idle);
   ...
   //CAM 4/3/16 attempting to add in setRegion, CutProt adn RegionCut for POL list
  fRegCutCmd = new G4UIcmdWithADoubleAndUnit("/Pol/physics/RegionCut",this);
  ...
  fRegCmd = new G4UIcmdWithAString("/Pol/physics/SetRegion",this);
...
PolPhysicsListMessenger::~PolPhysicsListMessenger()
{
  ...
  delete fpListCmd;
  delete flistCmd;
  delete pCutCmd; //CAM 4/3/16 attempting to add in setRegion, CutProt adn RegionCut for POL list
  delete fRegCutCmd;
  delete fRegCmd;
  delete fphysDir;
  delete fAyValue;
                                          ...
                                          G4String newValue)
{
  ...
  //CAM 4/3/16 attempting to add in setRegion, CutProt adn RegionCut for POL list
     ...
     fpPhysicsList->SetCutForProton(pCutCmd->GetNewDoubleValue(newValue));
 ...
 //CAM 4/3/16 attempting to add in setRegion, CutProt and RegionCut for POL list need to follow the ifs here.
