...
#include "G4Proton.hh"    //CAM 7/3/16
#include "G4RegionStore.hh" //CAM7/3/16
#include "G4RegionStore.hh" //CAM7/3/16
#include "G4ProductionCuts.hh" //CAM7/3/16
...
PolPhysicsList::PolPhysicsList() : G4VModularPhysicsList()
{
  ...
  cutForProton    = defaultCutValue; //CAM 7/3/16
  dump            = false;
  fIsSpinPrecN    = false;
...
{
  //std::cout << name << std::endl; //Shibolith
  if (verboseLevel>0)
...
void PolPhysicsList::SetCutForProton(G4double cut)
{
  cutForProton = cut;
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
//CAM 7/3/16
void PolPhysicsList::SetCutForRegion(G4String reg,G4double cut)
{
  ...
  else G4cout<<"PolPhysicsList::SetCutForRegion region "<<reg<<" not constructed"<<G4endl;
}
